#include "first_challenge_hayakawa/first_challenge_hayakawa.hpp"

FirstChallenge::FirstChallenge() : Node("first_challenge_hayakawa")
{
    // global変数を定義
...
double FirstChallenge::calc_distance()
{
    ...
    return std::hypot(odom_.value().pose.pose.position.x, odom_.value().pose.pose.position.y);
}
